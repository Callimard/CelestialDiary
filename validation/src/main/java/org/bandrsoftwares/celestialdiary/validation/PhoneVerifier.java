...
import org.bandrsoftwares.celestialdiary.validation.exception.FailToFormatPhoneNumber;
import org.bandrsoftwares.celestialdiary.validation.exception.NotPhoneNumberException;
import org.bandrsoftwares.celestialdiary.validation.exception.UnsupportedPhoneNumberException;
...
public class PhoneVerifier implements ConstraintValidator<ValidPhone, String> {
    ...
    public boolean isValid(String phoneNumberToVerify, ConstraintValidatorContext context) {
        if (phoneNumberToVerify != null) {
            return !phoneNumberToVerify.isBlank() && PhoneVerifier.isCorrectAndSupportedPhoneNumber(phoneNumberToVerify);
        } else
            return true;
    }
    ...
    private final Map<String, PhoneVerifier.PhoneFormatter> phoneFormatterMap;
    ...
    private void fillPhoneFormatter() {
        phoneFormatterMap.put(FRENCH_PHONE_CODE, new PhoneVerifier.FrenchPhoneFormatter());
        phoneFormatterMap.put(US_PHONE_CODE, new PhoneVerifier.USPhoneFormatter());
    }
    ...
    public static boolean isCorrectAndSupportedPhoneNumber(String supposedPhoneNumber) {
        ...
        if (isAcceptedString(supposedPhoneNumber) && !phone.isBlank()) {
            PhoneVerifier.PhoneFormatter phoneFormatter = searchPhoneFormatterFor(phone);
    ...
    private static PhoneVerifier.PhoneFormatter searchPhoneFormatterFor(String phone) {
        for (PhoneVerifier.PhoneFormatter phoneFormatter : PHONE_TOOL.phoneFormatterMap.values()) {
    ...
    public static String permissiveFormatPhone(String phone) throws NotPhoneNumberException, UnsupportedPhoneNumberException,
        ...
        for (PhoneVerifier.PhoneFormatter pFormatter : PHONE_TOOL.phoneFormatterMap.values()) {
    ...
    /**
     * Try to find a {@link PhoneVerifier.PhoneFormatter} which support the phone number. The first {@code PhoneVerifier.PhoneFormatter} find which support the phone
    ...
    private static class FrenchPhoneFormatter extends PhoneVerifier.PhoneFormatter {
        public FrenchPhoneFormatter() {
            super(FRANCE, FRENCH_PHONE_FORMAT_MASK, FRENCH_PHONE_FORMAT_REGEX, FRENCH_PHONE_CODE);
        }
    }

    private static class USPhoneFormatter extends PhoneVerifier.PhoneFormatter {
